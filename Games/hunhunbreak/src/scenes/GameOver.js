export class GameOver extends Phaser.Scene {
    constructor() {
        super('GameOver');
    }

    init(data) {
        this.finalScore = (data && data.score) ? data.score : 0;
    }

    create() {
        this.cameras.main.setBackgroundColor(0xff0000);

        this.add.image(512, 384, 'background').setAlpha(0.5);

        this.add.text(512, 384, 'Game Over', {
            fontFamily: 'Arial Black', fontSize: 64, color: '#ffffff',
            stroke: '#000000', strokeThickness: 8,
            align: 'center'
        }).setOrigin(0.5);

        // Display final score
        this.add.text(512, 460, 'Score: ' + (this.finalScore || 0), {
            fontFamily: 'Arial', fontSize: 32, color: '#ffffff',
            stroke: '#000000', strokeThickness: 6,
            align: 'center'
        }).setOrigin(0.5);

        // Restart button
        const restartText = this.add.text(512, 540, 'Restart', {
            fontFamily: 'Arial Black', fontSize: 36, color: '#ffffff',
            stroke: '#000000', strokeThickness: 6
        }).setOrigin(0.5).setInteractive({ useHandCursor: true });

        restartText.on('pointerup', () => {
            // Start the Game scene fresh; Game.create will initialize score and bricks
            this.scene.start('Game');
        });

        // Display mute image if loaded
        try {
            if (this.textures && this.textures.exists && this.textures.exists('mute')) {
                const img = this.add.image(512, 170, 'mute').setOrigin(0.5).setDepth(400);
                try {
                    const w = img.width || (img.frame && img.frame.width) || img.displayWidth;
                    const h = img.height || (img.frame && img.frame.height) || img.displayHeight;
                    if (w && h) {
                        const scale = 320 / Math.max(w, h);
                        img.setDisplaySize(Math.round(w * scale), Math.round(h * scale));
                    } else {
                        img.setDisplaySize(320 , 320);
                    }
                } catch (e) {}
            }
        } catch (e) {}
    }
}
